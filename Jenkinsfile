            ...
            steps{
                script{
                    if(!params.get('skipTests', false)) {
                        ...
                        sh "docker-compose -f tests/docker/docker-compose.test.yml run --rm test-templating pytest --junitxml=/coverage/pytest-report.xml --cov-report=xml:/coverage/coverage.xml --cov=${sonar_analyzed_dir}"
                    }
                }
            }
        }
