...
class TestGetTemplates:
    ...
    def test_obtain_all_template_info(self, client_file_storage):
        response = client_file_storage.get(self.GET_TEMPLATES_ENDPOINT)
    ...
    def test_obtain_template_info_by_id_ok(self, client_file_storage):
        tentative_template_id = 39
        assert tentative_template_id < NUMBER_OF_TEMPLATES

        response = client_file_storage.get(f"{self.GET_TEMPLATES_ENDPOINT}{tentative_template_id}")
        assert response.status_code == HTTPStatus.OK
    ...
    def test_obtain_template_info_by_id_not_found(self, client_file_storage):
        tentative_template_id = 200
        assert tentative_template_id > NUMBER_OF_TEMPLATES
        response = client_file_storage.get(self.GET_TEMPLATES_BY_ID_ENDPOINT.format(tentative_template_id))
        assert response.status_code == HTTPStatus.NOT_FOUND
        assert get_message(response) == template_not_found.format(tentative_template_id)

    def test_obtain_template_by_tags(self, client_file_storage):
        for i in range(NUMBER_OF_TEMPLATES):
            current_tag = f"tag{i}"
            tags = {"tags": [current_tag]}
            response = client_file_storage.get(self.GET_TEMPLATES_ENDPOINT, query_string=tags)
            assert response.status_code == HTTPStatus.OK
            assert len(response.json) == 1
            template_json = response.json[0]
            assert current_tag == template_json["tags"][0]

    def test_obtain_template_by_tags_empty(self, client_file_storage):
        template_id = 67
    ...
    def test_obtain_template_by_more_than_one_tag(self, client_file_storage):
        template_id = 32
        assert template_id < NUMBER_OF_TEMPLATES
        current_tag = f"tag{template_id}"
